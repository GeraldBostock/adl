...
vec3 calculate_directional_light(Sun sun, vec3 normal, vec3 view_direction)
{
	...
	vec3 specular = material.specular * spec * sun.specular;
	
	float diff = max(dot(normal, light_direction), 0.0);
	vec3 diffuse = diff * material.diffuse * sun.diffuse;
...
{	
	...
	vec3 directional_light_result = calculate_directional_light(sun, norm, view_direction);
	vec3 point_light_result = calculate_point_light(point_light, norm, fragment_position, view_direction);
	
	out_color = vec4(directional_light_result + point_light_result, 1.0);
}